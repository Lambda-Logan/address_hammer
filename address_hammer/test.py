...
class TestAddress(unittest.TestCase):
    ...
    def test_json(self):
        ...
        self.assertEqual(example_addresses, 
                         [json_reparse(a) for a in example_addresses])
    ...
    def test(self):
        ...
        self.assertEqual(2, len(HashableFactory\
        ...
        for a in example_addresses:
            ...
            for x, y in soft_sames + sames:
                for soft in SOFT_COMPONENTS:
                    self.assertEqual(a.replace(**{soft:x}), 
                                     a.replace(**{soft:y}))
                    self.assertNotEqual(a.replace(**{soft:"X"}), 
                                        a.replace(**{soft:"Y"}))

            for hard in HARD_COMPONENTS:
                for x, y in soft_sames:
                    self.assertNotEqual(a.replace(**{hard:x}), 
                                        a.replace(**{hard:y}))
                for x, y in sames:
...
class TestHammer(unittest.TestCase):
    ...
    def test(self):
        ...
        (ambigs_2, hammer)
        self.assertEqual(sorted(map(Address.Get.pretty, set(iter(hammer)))),
                         sorted(['001 E Street St Apt 1 City MI', 
                                 '001 E Street St Apt 0 City MI']))
