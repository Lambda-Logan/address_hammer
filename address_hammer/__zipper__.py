...
class Zipper(Generic[I, O]):
    ...
    def or_(
        ...
        for func in funcs:
            try:
                ...
                if len(results) > 0:
                    return self.merge(
                        Zipper(leftover=self.leftover.advance(1), results=results)
                    )
            except tuple(ex_types):
                pass
    ...
    def takewhile(
        ...
        return self.merge(Zipper(leftover=new_input, results=results))

    def test(self, name: str, result: Seq[int]):
        _r = list(self.realize().results)
        passed = list(result) == _r
        if not passed:
            raise Exception(f"zipper '{name}' failed at \n {_r}")

    def test_leftover(self, name: str, leftover: Seq[I]):
        _l = self.realize().leftover.as_str()
        passed = " ".join(map(str, leftover)) == _l
        if not passed:
            raise Exception(f"zipper '{name}' failed LEFTOVERS {leftover} \n {_l}")


def x(
    _z: Zipper[Seq[I], O], _funcs: Iter[Fn[[Zipper[I, O]], Zipper[I, O]]]
) -> Zipper[I, O]:
...
class Apply:
    ...
    @staticmethod
    def reduce(
        funcs: Iter[Fn[[Zipper[I, O]], Zipper[I, O]]]
    ) -> Fn[[Zipper[I, O]], Zipper[I, O]]:
