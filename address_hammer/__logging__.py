...
class log_parse_with(AbstractContextManager):
    def __init__(self, log : Fn[[Tuple[str,str]], None]):
        def logged(fn: Fn[[Any], Fn[[Zipper[str,ParseStep]], Zipper[str,ParseStep]]])->Fn[[Zipper[str,ParseStep]], Zipper[str,ParseStep]]:
            def _(*args: Any, **kwargs: Any)->Fn[[Zipper[str,ParseStep]], Zipper[str,ParseStep]]:
                z_z  = fn(*args, **kwargs)
                def __(z: Zipper[str,ParseStep])->Zipper[str,ParseStep]:
                    ...
                    rz: Zipper[str, ParseStep] = Zipper(leftover=leftover, results=iter(results))
                    return rz
                return __
            return _
